...
func NewWebAdapter(f engine.EngFactory, log bool) http.Handler {

	// TODO: abstract engine so you can use other template
	// engines and mux routers and middleware
	return loadGinAdapter(f, log)
}

func loadGinAdapter(f engine.EngFactory, log bool) http.Handler {
	var e *gin.Engine
	if log {
		e = gin.Default()
	} else {
